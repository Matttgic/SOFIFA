const fs = require('fs');
const { getPlayerDetailsCsvRow } = require('./services/parser-lite'); // version modifiée pour axios
...
async function download(fileToRead, fileToWrite) {
    const playerUrlList = fs.readFileSync(fileToRead, 'utf-8').trim().split('\n');
    if (!fs.existsSync(fileToWrite)) {
        fs.writeFileSync(fileToWrite, row_header);
    }
    ...
    for (let i = lastIndex + 1; i < playerUrlList.length; i++) {
        const url = playerUrlList[i];
        try {
            const row = await getPlayerDetailsCsvRow(url);
...
(async function start() {
    ...
    if (scanType === 'full') {
        ...
        console.error('❌ Mauvais argument : utilise "full", "test", "download-urls" ou "download-urls-test"');
    }
})(); 
